...
export async function getTimeLogs(_email?: string) {
  const res = await fetch("/api/v1/time-logs/mine");
...
export async function clockIn(_id?: string, _name?: string, _email?: string) {
  const res = await fetch("/api/v1/time-logs/clock-in", { method: "POST" });
  return handleResponse(res);
}

export async function clockOut(logId: string) {
  const res = await fetch("/api/v1/time-logs/clock-out", {
    method: "POST",
